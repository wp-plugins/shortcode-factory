<div class="scf-shortcode-wrapper">
	<h2 class="scf-shortcode-title" data-shortcode="scf-post-field">[scf-post-field]</h2>
	<p class="scf-shortcode-desc"><?=__('Outputs custom field of the post. If ID or Slug is provided, outputs the custom field of that particular post.', 'shortcode-factory')?></p>
	<div class="scf-controls-group">
		...
		<div class="scf-control">
			<label for="scf-control-name" class="scf-control-label"><?=__('Custom Field Name', 'shortcode-factory')?> <span class="required scf-right"><?=__('(required)', 'shortcode-factory')?></span></label>
			<input type="text" id="scf-control-name" name="scf-param-name" class="scf-control-text scf-param" data-param-name="name" data-required="1" placeholder="This field is required" />
		</div>
		...
		<div class="scf-control">
			...
			<span class="notes"><?=__('In case custom field has multiple values stored as an array or object. By default separated with a comma.', 'shortcode-factory')?></span>
		</div>
